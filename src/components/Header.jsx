    ...
    const menuItems = [
        { path: '/inicio', label: 'Inicio', icon: Home },
        {
            ...
            items: [
                ...
                { path: '/inventario', label: 'Inventario SICAR', icon: Store },
                { path: '/conciliacion-bancaria', label: 'Conciliacion Bancaria', icon: Landmark },
                { path: '/movimientos', label: 'Movimientos Contables', icon: FileText },
                { path: '/ajustes-manuales', label: 'Ajustes Manuales', icon: Calculator }
            ]
        },
        {
            key: 'operaciones',
            ...
            icon: Wallet,
            items: [
                { path: '/data-entry', label: 'Ingreso Manual', icon: FileText },
                { path: '/cierre-caja', label: 'Cierre de Caja', icon: Wallet },
                { path: '/depositos-transito', label: 'Depositos en Transito', icon: ArrowRightLeft },
                { path: '/confirmacion-deposito', label: 'Confirmar Deposito', icon: Landmark }
            ]
        },
        {
            key: 'clientes',
        ...
        },
        {
            ...
            icon: Building2,
            items: [
                { path: '/compras', label: 'Compras y Gastos', icon: ShoppingCart },
                        ...
                        {userMenuOpen && (
                            <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-lg shadow-xl border border-slate-200 py-2 z-50">
                                <div className="px-4 py-2 border-b border-slate-100">
                                    ...
                                    <p className="text-xs text-slate-500">{roleLabels[userRole] || 'Usuario'}</p>
                                </div>
                                <Link
                                ...
                                <div key={item.key}>
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${
                                            hasActiveChild ? 'bg-blue-600 text-white' : 'hover:bg-slate-700'
                                        }`}
                                    >
